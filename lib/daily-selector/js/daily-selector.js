...
DailySelectorHelper.prototype = {
  ...
  _createCalendar: function (calendarContainer, options) {
    ...
    btnPreviousMonth.addEventListener("click", function (event) {
      dailySelectorHelper._calendarNavigation(this, NAVIGATION.PREVIOUS);
    });
  ...
  _updateHeader: function (currentYear, currentMonth, currentDate, currentDay) {
    ...
    for (let i = 0; i < selectedYear.children.length; i++) {
      if (
        selectedYear.children[0].tagName.toLowerCase() === ELEMENTS.SPAN_ELEMENT
      ) {
    ...
    for (let i = 0; i < selectedDate.children.length; i++) {
      if (
        selectedDate.children[0].tagName.toLowerCase() === ELEMENTS.SPAN_ELEMENT
      ) {
...
Modal.prototype = {
  _registerEventListener: function (modalObj) {
    ...
    for (var i = 0; i < modalHelper.modalOpenElement.length; i++) {
      modalHelper.modalOpenElement[i].addEventListener(
        "click",
        function (event) {
          event.preventDefault();
    ...
    for (var i = 0; i < modalHelper.modalCloseElement.length; i++) {
      modalHelper.modalCloseElement[i].addEventListener(
        "click",
        function (event) {
          modalHelper._toggleModal();
        }
    ...
    let closeOnClickOutsideModal =
      modalHelper.closeOptions.closeOnClickOutsideModal;
