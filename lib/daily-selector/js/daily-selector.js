...
DailySelector.prototype = {
  initialize: function (options) {
    ...
    function _createYearList(options) {
      ...
      const minYear = (options?.year?.min === undefined || isNaN(options.year.min)) ? currentTodayYear - 10 : options.year.min;
      const maxYear = (options?.year?.max === undefined || isNaN(options.year.max)) ? currentTodayYear : options.year.max;
