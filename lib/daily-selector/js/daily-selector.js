...
DailySelector.prototype = {
  initialize: function (options) {
    ...
    function _createClassicContainer(containerId) {
      ...
      let parentDiv = document.createElement(CONSTANTS.CONTAINER_ELEMENT);
        ...
        if (originalParent.children.length > 0) {
          for (let i = 0; i < originalParent.children.length; i++) {
            if (originalParent.children[i].id === dateElement.id) {
              ...
              let container = document.createElement(CONSTANTS.CONTAINER_ELEMENT);
    ...
    function _createCalendar(calendarContainer) {
      let calendarHeader = document.createElement(CONSTANTS.CONTAINER_ELEMENT);
      calendarHeader.id = "calendar-header";
      calendarHeader.classList.add("calendar-header");
      let dateDiv = document.createElement(CONSTANTS.CONTAINER_ELEMENT);
      dateDiv.id = "selected-date";
      dateDiv.classList.add("date");
      let spanDate = document.createElement("span");
      dateDiv.appendChild(spanDate);
      let yearDiv = document.createElement(CONSTANTS.CONTAINER_ELEMENT);
      ...
      let calendarNavigation = document.createElement(CONSTANTS.CONTAINER_ELEMENT);
      ...
      let calendarBody = document.createElement(CONSTANTS.CONTAINER_ELEMENT);
      calendarBody.id = "calendar-days";
      calendarBody.classList.add("calendar-days");

      let calendarFooter = document.createElement(CONSTANTS.CONTAINER_ELEMENT);
      calendarFooter.classList.add("calendar-footer");

      let calendarAction = document.createElement(CONSTANTS.CONTAINER_ELEMENT);
    ...
    function _updateCalendar(currentYear, currentMonth) {
      ...
      for (let i = 1; i <= daysInMonth; i++) {
        const dayDiv = document.createElement(CONSTANTS.CONTAINER_ELEMENT);
      ...
      for (let i = 0; i < selectedDate.children.length; i++) {
        if (selectedDate.children[0].tagName.toLowerCase() === "span") {
          selectedDate.children[0].textContent = `${currentDay}, ${currentDate} ${currentMonth}`;
        }
      }
    }
