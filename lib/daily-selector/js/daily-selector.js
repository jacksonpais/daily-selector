...
DailySelectorHelper.prototype = {
    ...
    _createContainer: function (options) {
        switch (options.theme) {
            case "standalone":
                return dailySelectorHelper._createStandaloneContainer(options);
            case "popup":
                return dailySelectorHelper._createPopupContainer(options);
            case "inline":
            default:
                return dailySelectorHelper._createInlineContainer(options);
        }
    },

    // create inline ui
    _createInlineContainer: function (options) {
        let elementId = options.elementId;
        let containerId = `${elementsMetaData.container.id}${elementId}`;
        let parentDiv = dailySelectorHelper._createElement(ELEMENTS.DIV_ELEMENT, elementId);
    ...
    // create standalone
    _createStandaloneContainer: function (options) {
        let elementId = options.elementId;
        let dateElement = document.getElementById(elementId);
        let container = dailySelectorHelper._createElement(ELEMENTS.DIV_ELEMENT, elementId);
        container.id = `${elementsMetaData.container.id}${elementId}`;
    ...
    // create popup ui
    _createPopupContainer: function (options) {
        let elementId = options.elementId;
        let containerId = `${elementsMetaData.container.id}${elementId}`;
        let parentDiv = dailySelectorHelper._createElement(ELEMENTS.DIV_ELEMENT, elementId);
        let dateElement = document.getElementById(elementId);
            ...
            if (originalParent.children.length > 0) {
                for (let i = 0; i < originalParent.children.length; i++) {
                    if (originalParent.children[i].id === dateElement.id) {
                        ...
                        if (options?.popup?.modal !== undefined) {
                            modalContent = document.getElementById(options.popup.modal);
                        }
                        else {
                            modalContent = modalHelper._createModal(container);
                        }
    ...
    // open close modal
    _actionModal: function (modal, MODAL_ACTIONS) {
    ...
    _toggleCalendarViews: function (calendarContainer) {
        ...
        if (viewId === view_1.id) {
            view_2.classList.remove("hide");
            dailySelectorHelper.dailySelectorHelper._setAttributesToElement(calendarContainer, "display", view_2.id);
    ...
    _createCalendar: function (calendarContainer, options) {
        ...
        btnClear.addEventListener("click", function (event) {
            ...
            let dateElement = document.getElementById(elementId);
            dateElement.value = "";
            dailySelectorHelper._resetDateElement(dateElement, options.theme);
        });
    ...
    _createYearList: function (options) {
        ...
        for (let year = minYear; year <= maxYear; year++) {
            ...
            dailySelectorHelper._setAttributesToElement(yearItem, "currentYear", year);
            ...
            for (let month = 0; month < 12; month++) {
                ...
                dailySelectorHelper._setAttributesToElement(monthItem, "currentMonth", month);
                dailySelectorHelper._setAttributesToElement(monthItem, "currentYear", year);
            ...
            yearHeader.addEventListener("click", function (event) {
                event.preventDefault();
                let currentYear = parseInt(
                    dailySelectorHelper._getAttributesFromElement(this.parentElement, "currentYear")
                );
    ...
    // validates date
    _isDateValid: function (dateString) {
        const date = Date.parse(dateString);
        return !isNaN(date);
    },

    // set calendar's navigation previous month and next month
    _calendarNavigation: function (e, navigation) {
    ...
    _updateCalendar: function (options, currentYear, currentMonth) {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            dailySelectorHelper._setAttributesToElement(dayDiv, "currentMonth", currentMonth);
            dailySelectorHelper._setAttributesToElement(dayDiv, "currentYear", currentYear);
            dailySelectorHelper._setAttributesToElement(dayDiv, "currentDate", i);
            let dt = new Date(currentYear, currentMonth, i);
            dailySelectorHelper._setAttributesToElement(dayDiv, "currentDay", dt.getDay());
    ...
    _closeBoxOutside: function (event) {
            ...
            if (event.target.id !== elementsMetaData.modal.id) {
                if (container !== null) {
                    if (
                        ...
                        //dailySelectorHelper._resetDateElement(dateElement, options.theme);
                        document.removeEventListener("click", dailySelectorHelper._closeBoxOutside);
                    }
                }
            }
        }
    ...
    _resetDateElement: function (dateElement, theme) {
...
var Modal = function () {
    ...
    this.overlay;
    this.closeOptions;
    this.modalOpenElement = document.querySelectorAll('.modal-open');
    this.modalCloseElement = document.querySelectorAll('.modal-close');
  }
  
  Modal.prototype = {
    _registerEventListener: function (modalObj) {
        modalHelper.modal = document.getElementById(modalObj.modal);
        modalHelper.overlay = document.getElementById(modalObj.overlay);
        modalHelper.closeOptions = modalObj.closeOptions;
  
        for (var i = 0; i < modalHelper.modalOpenElement.length; i++) {
            modalHelper.modalOpenElement[i].addEventListener('click', function (event) {
                event.preventDefault()
                modalHelper._toggleModal()
            })
        }
  
        for (var i = 0; i < modalHelper.modalCloseElement.length; i++) {
            modalHelper.modalCloseElement[i].addEventListener('click', function (event) {
                modalHelper._toggleModal()
            })
        }
  
        let closeOnClickOutsideModal = modalHelper.closeOptions.closeOnClickOutsideModal;
        let closeOnKeyboardKeys = modalHelper.closeOptions.closeOnKeyboardKeys;
  
        if (closeOnClickOutsideModal === true) {
            modalHelper.overlay.addEventListener('click', function (event) {
                modalHelper._closeModal()
            })
        }
    ...
    _loadElement: function (contentElement) {
        let detailsPanel = document.getElementById("select-modal");
        detailsPanel.innerHTML = "";
        detailsPanel.appendChild(contentElement);
    },
    _toggleModal: function () {
        const body = document.querySelector('body')
        modalHelper.overlay.classList.toggle('opacity-0')
        modalHelper.overlay.classList.toggle('pointer-events-none')
        body.classList.toggle('modal-active')
    },
    _openModal: function () {
        const body = document.querySelector('body')
        modalHelper.overlay.classList.add('opacity-0')
        modalHelper.overlay.classList.add('pointer-events-none')
        body.classList.add('modal-active')
    },
    _closeModal: function () {
        const body = document.querySelector('body')
        modalHelper.overlay.classList.remove('opacity-0')
        modalHelper.overlay.classList.remove('pointer-events-none')
        body.classList.remove('modal-active')
    },
    _createModal: function (content) {
      let elementId = dailySelectorHelper._getElementId(content);
      let modal = dailySelectorHelper._createElement(ELEMENTS.DIV_ELEMENT, elementId);
      modal.id = elementsMetaData.modal.id;
      modal.className = elementsMetaData.modal.class;
      let modalOverlay = dailySelectorHelper._createElement(ELEMENTS.DIV_ELEMENT, elementId);
      let modalContent = dailySelectorHelper._createElement(ELEMENTS.DIV_ELEMENT, elementId);
      modalContent.className = elementsMetaData.modalContent.class;
      modal.appendChild(modalOverlay);
      modal.appendChild(modalContent);
      document.body.appendChild(modal);
      //dailySelectorHelper._actionModal(modal, MODAL_ACTIONS.OPEN);
      return modalContent;
  },
  ...
  // <div class="modal opacity-0 pointer-events-none fixed w-full h-full top-0 left-0 flex items-center justify-center z-10" id="list-modal-overlay">
  //     <div class="modal-overlay absolute w-full h-full bg-gray-900 opacity-50"></div>
  
  //     <div class="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto" id="list-modal">
