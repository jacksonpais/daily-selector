  ...
  initialize: function (options) {
    ...
    function _createContainer(options) {
      ...
      switch (options.theme) {
        ...
        case "inline":
        default:
          _createInlineContainer(options);
          break;
      }
    }
    ...
    function _initializeViews(
      ...
      if (view_1.id === initialViewId) {
        view_1.classList.remove("hide");
        view_2.classList.add("hide");
      } else if (view_2.id === initialViewId) {
        view_2.classList.remove("hide");
        view_1.classList.add("hide");
      }
    }

    function _toggleCalendarViews(
      calendarContainer,
      initialViewId_1,
      initialViewId_2
    ) {
      let view_1 = document.getElementById(initialViewId_1);
      let view_2 = document.getElementById(initialViewId_2);

      view_1.classList.add("hide");
      view_2.classList.add("hide");
      let viewId = _getAttributesFromElement(calendarContainer, "display");
      if (viewId === initialViewId_1) {
        view_2.classList.remove("hide");
        _setAttributesToElement(calendarContainer, "display", initialViewId_2);
      } else if (viewId === initialViewId_2) {
        view_1.classList.remove("hide");
        _setAttributesToElement(calendarContainer, "display", initialViewId_1);
      }
    }
    ...
    function _createCalendar(calendarContainer, options) {
      ...
      let yearNavigation = _createElement(ELEMENTS.DIV_ELEMENT, elementId);
      yearNavigation.classList.add("year-navigation");
      let btnPreviousYear = _createElement(ELEMENTS.BUTTON_ELEMENT, elementId);
      btnPreviousYear.id = "prev-year";
      btnPreviousYear.innerHTML = "&lt;";
      btnPreviousYear.classList.add("icon");
      let btnNextYear = _createElement(ELEMENTS.BUTTON_ELEMENT, elementId);
      btnNextYear.id = "next-year";
      btnNextYear.innerHTML = "&gt;";
      btnNextYear.classList.add("icon");
      let yearList = _createElement(ELEMENTS.DIV_ELEMENT, elementId);
      yearList.classList.add("year-list");


      yearNavigation.appendChild(btnPreviousMonth);
      let title1 = _createElement(ELEMENTS.H2_ELEMENT, elementId);
      title1.id = "year-title";
      yearNavigation.appendChild(title1);
      yearNavigation.appendChild(btnNextMonth);
      ...
      // Past year navigation
      let calendarPastYears = _createElement(ELEMENTS.DIV_ELEMENT, elementId);
      calendarPastYears.id = "calendar-past-years";
      calendarPastYears.classList.add("calendar-past-years");
      // createYearList(yearList);
      calendarPastYears.appendChild(yearNavigation);
      calendarPastYears.appendChild(yearList);

      calendarContainer.appendChild(calendarPastYears);
    ...
    function createYearList(yearContainer) {
      ...
      for (let year = 2000; year <= currentYear; year++) {
        ...
        for (let month = 0; month < 12; month++) {
          ...
          monthItem.setAttribute("tabindex", "0"); // Make month item focusable
        ...
        // Focus on clicked month item
        yearGrid.addEventListener("click", function (e) {
          const clickedMonth = e.target;
          if (clickedMonth.classList.contains("month-item")) {
            clickedMonth.focus();
          }
        });
