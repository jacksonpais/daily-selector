        ...
        if (originalParent.children.length > 0) {
          for (let i = 0; i < originalParent.children.length; i++) {
            if (originalParent.children[i].id === dateElement.id) {
              ...
              //_createCalendar(container, options);
              let modalContent = modalHelper._createModal(container);
...
Modal.prototype = {
  ...
  _createModal: function (content) {
    let elementId = dailySelector._getElementId(content);
    let modal = dailySelector._createElement(ELEMENTS.DIV_ELEMENT, elementId);
    modal.id = elementsMetaData.modal.id;
    modal.className = elementsMetaData.modal.class;
    let modalOverlay = dailySelector._createElement(ELEMENTS.DIV_ELEMENT, elementId);
    let modalContent = dailySelector._createElement(ELEMENTS.DIV_ELEMENT, elementId);
